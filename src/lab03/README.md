...
**Сценарий 2 — Вывод информации (переопределенный __str__)**  
Как работает: При вызове print() для каждого объекта вызывается свой метод __str__, который выводит общую информацию из базового класса и специфичную из дочернего.
...
Как работает: Коллекция StudentGroup может хранить объекты всех типов, так как они наследуются от Student. Методы коллекции работают корректно.
